...
test.describe('系统规则完整流程', () => {
  test('创建 → 验证显示 → 持久化 → 修改 → 删除（LOAD类型）', async ({ page }) => {
    ...
    // 步骤 2: 验证规则在列表中显示
    // ============================================
    await expect(page.locator('tr:has-text("LOAD"), tr:has-text("系统负载")').first()).toBeVisible({ timeout: 5000 });
    ...
    const editButton = page
      .locator('tr:has-text("LOAD") button[aria-label="编辑"], tr:has-text("系统负载") button[aria-label="编辑"]')
      .first();
    if (await editButton.isVisible({ timeout: 2000 })) {
      ...
      // 验证修改成功
      const row = page.locator('tr:has-text("LOAD"), tr:has-text("系统负载")');
    ...
    const deleteButton = page
      .locator('tr:has-text("LOAD") button[aria-label="删除"], tr:has-text("系统负载") button[aria-label="删除"]')
      .first();
  ...
  test('创建 CPU 类型规则', async ({ page }) => {
    ...
    // 验证创建成功
    await expect(page.locator('tr:has-text("CPU"), tr:has-text("CPU使用率")').first()).toBeVisible({ timeout: 5000 });

    // 清理：删除规则
    const deleteButton = page
      .locator('tr:has-text("CPU") button[aria-label="删除"], tr:has-text("CPU使用率") button[aria-label="删除"]')
      .first();

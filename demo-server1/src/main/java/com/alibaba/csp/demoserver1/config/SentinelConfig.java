package com.alibaba.csp.demoserver1.config;
...
public class SentinelConfig {
     ...
     * Token Server 地址
     */
    @Value("${sentinel.cluster.client.server-host:token-server}")
     ...
     * 集群客户端请求超时时间
     */
    @Value("${sentinel.cluster.client.request-timeout:20000}")
    ...
    public void onApplicationReady() {
        initClusterClientMode();
    }
    
    /**
     * 初始化集群客户端模式
     */
    private void initClusterClientMode() {
        try {
            logger.info("==============================================");
            logger.info("Initializing Cluster Client Mode");
            logger.info("Token Server: {}:{}", tokenServerHost, tokenServerPort);
            logger.info("Request Timeout: {} ms", requestTimeout);
            logger.info("==============================================");
            
            // 1. 设置 Token Server 地址
            ClusterClientAssignConfig assignConfig = new ClusterClientAssignConfig();
            assignConfig.setServerHost(tokenServerHost);
            assignConfig.setServerPort(tokenServerPort);
            ClusterClientConfigManager.applyNewAssignConfig(assignConfig);
            logger.info("✓ Token Server address configured: {}:{}", tokenServerHost, tokenServerPort);
            
            // 2. 设置客户端配置
            ClusterClientConfig clientConfig = new ClusterClientConfig();
            clientConfig.setRequestTimeout(requestTimeout);
            ClusterClientConfigManager.applyNewConfig(clientConfig);
            logger.info("✓ Client config applied (timeout: {} ms)", requestTimeout);
            
            // 3. 切换到集群客户端模式
            ClusterStateManager.applyState(ClusterStateManager.CLUSTER_CLIENT);
            logger.info("✓ Cluster Client Mode ACTIVATED");
            
            // 4. 验证模式
            int currentMode = ClusterStateManager.getMode();
            if (currentMode == ClusterStateManager.CLUSTER_CLIENT) {
                logger.info("==============================================");
                logger.info("✅ Cluster Client initialized successfully!");
                logger.info("   Mode: CLUSTER_CLIENT (2)");
                logger.info("   Server: {}:{}", tokenServerHost, tokenServerPort);
                logger.info("==============================================");
            } else {
                logger.error("❌ Failed to activate Cluster Client mode - current mode: {}", currentMode);
            }
            
        } catch (Exception e) {
            logger.error("❌ Failed to initialize Cluster Client mode", e);
            throw new RuntimeException("Cluster Client initialization failed", e);
        }
    }

 ...
 * - 系统规则：高负载接口测试系统保护
 * - 授权规则：需要来源识别的接口
 * - 服务调用链：demo-server1 -> demo-server2
 * 
...
public class DemoController {
    ...
    @Value("${demo.server2.url:http://demo-server2:8083}")
    private String demoServer2Url;
    ...
    /**
     * 服务调用链测试接口
     * demo-server1 -> demo-server2
     * 用于测试跨服务调用场景下的限流和监控
     */
    @GetMapping("/service-chain")
    @SentinelResource(value = "/api/service-chain", blockHandler = "serviceChainBlockHandler")
    public ResponseEntity<Map<String, Object>> serviceChain() {
        Map<String, Object> result = new HashMap<>();
        result.put("service", "demo-server1");
        result.put("timestamp", System.currentTimeMillis());
        
        try (Entry entry = SphU.entry("/service/call-demo-server2")) {
            // 调用 demo-server2
            @SuppressWarnings("unchecked")
            ResponseEntity<Map> response = restTemplate.getForEntity(
                demoServer2Url + "/api/hello", 
                Map.class
            );
            result.put("demo-server2-response", response.getBody());
            result.put("chain-status", "success");
        } catch (BlockException e) {
            result.put("chain-status", "blocked by sentinel");
            result.put("blockType", e.getClass().getSimpleName());
        } catch (Exception e) {
            result.put("chain-status", "error");
    ...
    public ResponseEntity<Map<String, Object>> serviceChainBlockHandler(BlockException ex) {
        Map<String, Object> result = new HashMap<>();
        result.put("message", "Service chain blocked");
        result.put("blockType", ex.getClass().getSimpleName());
        return ResponseEntity.status(429).body(result);
    }
}

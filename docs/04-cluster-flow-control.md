...
本项目**仅支持独立模式（Standalone）**，这是生产环境的最佳实践：

| 对比项       | 单机限流                      | 集群限流（独立模式）           |
| ------------ | ----------------------------- | ------------------------------ |
| 计数方式     | 每个实例独立计数              | Token Server 全局计数          |
| 部署方式     | 无需额外组件                  | 独立部署 Token Server          |
| 适用场景     | 无状态服务、不需要精确限流    | 需要精确全局限流的生产环境     |
| 可靠性       | 高（无依赖）                  | 高（独立部署，不受业务应用影响） |

> **设计说明**：本项目不支持嵌入式模式，避免了 Token Server 与业务应用耦合带来的稳定性问题。
...
> **说明**：
> - **Token Server**：独立部署的令牌服务器，负责全局流量统计和令牌分发
> - **Token Client**：项目提供的示例应用，演示业务应用如何接入集群流控
> - **业务应用**：您的实际业务服务，集成 Sentinel SDK 后作为 Client 角色接入
